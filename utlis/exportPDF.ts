...
export function exportToPDF({
  ...
  doc.text("Spendly — Financial Report", 14, 18);
  ...
  doc.text(`৳${totalBudget.toLocaleString()}`, 20, 63);
  doc.setTextColor(239, 68, 68);
  doc.text(`৳${totalSpent.toLocaleString()}`, 80, 63);
  doc.setTextColor(
    totalRemaining < 0 ? 239 : 16,
    totalRemaining < 0 ? 68 : 185,
    totalRemaining < 0 ? 68 : 129,
  );
  doc.text(`৳${totalRemaining.toLocaleString()}`, 140, 63);
  ...
  autoTable(doc, {
    startY: finalY + 4,
    head: [["Title", "Category", "Amount (৳)", "Month", "Year"]],
    body: expenses.map((e) => [
      e.title,
      e.category || "Other",
  ...
  doc.text(
    `Generated by Spendly • ${new Date().toLocaleDateString()}`,
    pageWidth / 2,
